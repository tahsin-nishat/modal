        ...
        for order, ms in modes.items():
            filtered_modes[order] = [mode for mode in ms
                                     if ((mode.eigenvalue.real < 0.)
                                        and
                                        mode.eigenvalue.imag > 0.)]
            """if any([
                (mode.eigenvalue.imag > 0.
             and abs(mode.mp / np.pi * 180)<80)
              or (mode.eigenvalue.imag > 0.
             and abs(mode.mp / np.pi) * 180>100) #changed this portion might change later
                    ])]"""
