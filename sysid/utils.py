...
class Beam(object):
    ...
    def _get_K(self):
        ...
        Msq = self.M** (-0.5);
        Fi, _ = linalg.eig(Msq * K * Msq)

        M2 = (Fi.T)*Fi;
        K2 = (Fi.T)*(Msq*K*Msq)*Fi;
        C2 = np.zeros((len(Kisi), len(Kisi)));
        for j in range(len(Kisi)):
            C2[j, j] = 2 * Kisi(j) * ((K2[j, j]) ** 0.5);

        fiTinv = (Fi.T)\eye(DoF);
        fiinv = (Fi)\eye(DoF);
        Mxqinv = (Msq)\eye(DoF);

        M = Mxqinv * (fiTinv * M2 * fiinv) * Mxqinv;
        C = Mxqinv * (fiTinv * C2 * fiinv) * Mxqinv;
        K = Mxqinv * (fiTinv * K2 * fiinv) * Mxqinv;
        """
    ...
    def simulate(self, t, F=None, d0=None, v0=None):
        ...
        F : 2d array, optional
            ...
            if None.
        d0, v0 : 1d array, optional
            Initial displacement and velocity vector. Zeros is assumed
            if None.
        ...
        _, y, _ = signal.lsim(sys, U, t, X0=x0) # returns time values, system response, time evolution of the state vector
        y = y.T
        A = y[:n, :]
        V = y[n:2*n, :]
        D = y[2*n:, :]
